  ...
  (async () => {
    ...
    feed.items.forEach((item) => {
      if (item.title.includes("Monthly News")) {
        let month = item.title.split(" ")[3];

        console.log(item.title + ":" + item.link);
        twit.getTweet(`${year}-${month}`).then(function (result) {
          if (result == false)
            fetch(item.link)
              .then((resp) => resp.text())
              .then((body) => {
                ...
                twit.sendTweetStats(imagePath, resultMap, `#linuxmint donation stats for ${year}-${month}:`);
              });
        });
      }
    });
